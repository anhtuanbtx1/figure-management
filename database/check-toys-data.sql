...
SELECT COUNT(*) as TotalToys FROM ManagementStore.dbo.Toys;

-- 2. Check active toys count
PRINT '';
PRINT '2. âœ… Active toys count:';
SELECT COUNT(*) as ActiveToys FROM ManagementStore.dbo.Toys WHERE IsActive = 1;
...
SELECT 
    ...
    t.IsActive
FROM ManagementStore.dbo.Toys t
LEFT JOIN ManagementStore.dbo.ToyCategories c ON t.CategoryId = c.Id AND c.IsActive = 1
LEFT JOIN ManagementStore.dbo.ToyBrands b ON t.BrandId = b.Id AND b.IsActive = 1
...
BEGIN
    ...
    END TRY
    BEGIN CATCH
        PRINT 'âŒ Stored procedure error: ' + ERROR_MESSAGE();
    END CATCH
END
ELSE
BEGIN
    PRINT 'âŒ sp_GetToysForFrontend does not exist';
END
...
BEGIN
    ...
    PRINT 'Fix: UPDATE ManagementStore.dbo.Toys SET IsActive = 1;';
END
